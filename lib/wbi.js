...
async function fetchWbiKeys() {
  const now = Date.now();
  if (imgKey && subKey && now - lastUpdateTime < 3600000) {
  ...
  try {
    ...
    console.warn('Failed to fetch WBI keys, using fallback:', error);
  }

  imgKey = '76e40f659f335a94';
  subKey = '77f90b0728178098';
  lastUpdateTime = now;
  return { imgKey, subKey };
}

async function signParams(params) {
  const { imgKey: ik, subKey: sk } = await fetchWbiKeys();
  const mixinKey = getMixinKey(ik + sk);
  const currTime = Math.round(Date.now() / 1000).toString();
  const chrFilter = /[!'()*]/g;
  ...
  const queryParts = sortedKeys.map(key => {
    const value = signedParams[key].toString().replace(chrFilter, '');
